...
ConstantFloat* ConstantFloat::get(float val, Module* m){
    if(cached_float.find(std::make_pair(val, m))!=cached_float.end())
        ...
        return (cached_float[std::make_pair(val, m)] = 
            std::unique_ptr<ConstantFloat>(new ConstantFloat(m->get_float_type(), val)))
            .get();
}
ConstantArray* ConstantArray::get(std::vector<int> &array, Module* m){
    std::vector<Constant*> array_;
    array_.reserve(array.size());
    for(auto& val : array)array_.push_back(ConstantInt::get(val, m));
    return new ConstantArray(m->get_array_type(m->get_int_type(), {(unsigned)array.size()}), array_);
}
ConstantArray* ConstantArray::get(std::vector<float> &array, Module* m){
    std::vector<Constant*> array_;
    array_.reserve(array.size());
    for(auto& val : array)array_.push_back(ConstantFloat::get(val, m));
    return new ConstantArray(m->get_array_type(m->get_float_type(), {(unsigned)array.size()}), array_);
}